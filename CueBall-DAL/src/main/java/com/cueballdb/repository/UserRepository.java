...
public interface UserRepository extends JpaRepository<User, Integer> {
    ...
    @Query("SELECT b FROM User b where b.id<>'1' and b.enable = true ORDER BY b.id DESC")
    ...
    @Modifying
    @Query("UPDATE User u SET u.enable = ?2, u.modifiedAt = current_timestamp WHERE id = ?1")
    int softDeleteUser(int id, boolean enable);
}
